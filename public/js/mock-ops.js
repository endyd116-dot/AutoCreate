...
(function () {
  ...
  const seed = () => ({
    ...
    runners: fresh ? [] : [
      { id: 1, kind: "managed", name: "farm-01", status: "online", load: 0.62, assignedAccounts: 18, jobsWaiting: 4, lastSeenAt: iso(now - 20e3), version: "1.0.3" },
      { id: 2, kind: "managed", name: "farm-02", status: "online", load: 0.21, assignedAccounts: 7, jobsWaiting: 0, lastSeenAt: iso(now - 40e3), version: "1.0.3" },
      { id: 3, kind: "managed", name: "farm-03", status: "offline", load: 0, assignedAccounts: 0, jobsWaiting: 0, lastSeenAt: iso(now - 9 * 3600e3), version: "1.0.2" },
      { id: 901, kind: "own", name: "집 PC", tenantName: "모의", status: "offline", load: 0, assignedAccounts: 2, jobsWaiting: 2, lastSeenAt: iso(now - 2 * 3600e3), version: "1.0.3" },
      { id: 902, kind: "own", name: "사무실", tenantName: "요리하는 집", status: "online", load: 0.1, assignedAccounts: 5, jobsWaiting: 0, lastSeenAt: iso(now - 30e3), version: "1.0.3" },
    ],
    canary: fresh ? [] : [0, 1, 2, 3, 4, 5, 6].flatMap((d) => [{ day: ymd(-d), channel: "naver_blog", ok: d !== 2, ranAt: iso(now - d * 86400e3 - 4 * 3600e3), shotKey: `canary/${ymd(-d)}/naver.png`, ...(d === 2 ? { error: "selector_changed: 발행 버튼을 못 찾음" } : {}) }, { day: ymd(-d), channel: "tistory", ok: d === 5 ? null : true, ranAt: iso(now - d * 86400e3 - 4 * 3600e3), ...(d === 5 ? { error: "세션 없음 · 판정 불가" } : { shotKey: `canary/${ymd(-d)}/tistory.png` }) }]),
    ai: { roles: [{ role: "writer", current: "text-current", candidate: "text-next", canaryPct: 10, updatedAt: iso(now - 2 * 86400e3) }, { role: "director", current: "json-current", canaryPct: 0, updatedAt: iso(now - 20 * 86400e3) }, { role: "topics", current: "search-current", canaryPct: 0, updatedAt: iso(now - 20 * 86400e3) }, { role: "image", current: "image-current", candidate: "image-next", canaryPct: 0, updatedAt: iso(now - 9 * 86400e3) }], candidates: [{ model: "text-next", tested: { text: true, json: true, googleSearch: true, image: false } }, { model: "image-next", tested: { text: false, json: false, googleSearch: false, image: true } }], costCap: { krwPerDay: 30000 }, updateMode: "manual", lastWatchAt: iso(now - 3 * 86400e3) },
    ...
    disclosure: { text: "이 포스팅은 쿠팡 파트너스 활동의 일환으로, 이에 따른 일정액의 수수료를 제공받습니다.", updatedAt: iso(now - 40 * 86400e3), updatedBy: "두현" },
    notices: fresh ? [] : [
      ...
      { id: 802, kind: "notice", title: "9월 25일 새벽 2시 점검(10분)", body: "점검 중에는 화면이 잠깐 안 열려요. 예약 발행은 영향 없어요.", startsAt: iso(now + 5 * 86400e3), endsAt: iso(now + 11 * 86400e3), plans: [], active: true },
    ],
    audit: fresh ? [] : [
      { id: 1, action: "ops.impersonate.start", actor_type: "operator", actor_id: 2, tenant_id: 3, risk_level: "high", created_at: iso(now - 50 * 60e3), detail: "팁스고 · 결제 문의 대응" },
      { id: 2, action: "ops.coins.grant", actor_type: "operator", actor_id: 2, tenant_id: 1, risk_level: "medium", created_at: iso(now - 5 * 3600e3), detail: "+30 온보딩 지원" },
      { id: 3, action: "billing_attempt", actor_type: "system", actor_id: null, tenant_id: 3, risk_level: "medium", created_at: iso(now - 20 * 3600e3), detail: "starter 청구 실패(카드 한도 초과) · 재시도 2/3" },
      { id: 4, action: "ops.price_event.create", actor_type: "operator", actor_id: 1, tenant_id: null, risk_level: "high", created_at: iso(now - 61 * 86400e3), detail: "pro 45,000 → 49,000 · 10/1" },
      { id: 5, action: "ad_media_state_claimed", actor_type: "user", actor_id: 1, tenant_id: 1, risk_level: "low", created_at: iso(now - 30 * 60e3), detail: "adpost applied · @cook_a" },
    ],
  });

  let S; try { S = JSON.parse(sessionStorage.getItem(KEY) || "null"); } catch { S = null; }
  if (!S || fresh || qs.get("reset") === "1" || !S.tickets) { S = seed(); save(); }
  ...
  const R = {
    ...
    /* ── 러너 팜 ── */
    "ops-runners": () => (need("super_admin") ? forbid() : { ok: true, runners: S.runners }),
    "ops-runner-assign": (b) => { const r = S.runners.find((x) => x.id === Number(b.runnerId)); if (!r) return err("runnerId", "러너가 없어요.", { status: 404 }); if (r.kind !== "managed") return err("kind", "고객 PC 러너는 배정할 수 없어요."); r.assignedAccounts += Number(b.accounts || 1); return { ok: true, runner: r }; },
    "ops-canary": () => (need("super_admin") ? forbid() : { ok: true, runs: S.canary }),
    /* ── AI ── */
    "ops-ai-models": () => { if (need("super_admin")) return forbid(); return { ok: true, ...S.ai }; },
    "ops-ai-apply": (b) => { if (need("super_admin")) return forbid(); const r = S.ai.roles.find((x) => x.role === b.role); if (!r) return err("role", "역할이 없어요."); if (b.chain) { if (Number(b.canaryPct) >= 100) { r.current = b.chain; delete r.candidate; r.canaryPct = 0; } else { r.candidate = b.chain; r.canaryPct = Number(b.canaryPct) || 10; } r.updatedAt = iso(Date.now()); } return { ok: true, role: r }; },
    "ops-ai-rollback": (b) => { if (need("super_admin")) return forbid(); const r = S.ai.roles.find((x) => x.role === b.role); if (!r) return err("role", "역할이 없어요."); delete r.candidate; r.canaryPct = 0; r.updatedAt = iso(Date.now()); return { ok: true, role: r }; },
    "ops-ai-cost-cap": (b) => { if (need("super_admin")) return forbid(); if (b.krwPerDay != null) S.ai.costCap.krwPerDay = Number(b.krwPerDay); if (b.updateMode) S.ai.updateMode = b.updateMode; return { ok: true, costCap: S.ai.costCap, updateMode: S.ai.updateMode }; },
    /* ── 채널 ── */
    "ops-channels": (b) => { if (b && b.key) { if (need("admin")) return forbid(); const c = S.channels.find((x) => x.key === b.key); if (!c) return err("key", "채널이 없어요."); if (b.status) { if (!["active", "planned", "down"].includes(b.status)) return err("status", "상태는 active·planned·down 중 하나예요."); c.status = b.status; } if (b.bestHours) c.bestHours = b.bestHours; return { ok: true, channel: c }; } return { ok: true, channels: S.channels }; },
    "ops-disclosure": (b) => { if (b && b.text != null) { if (need("super_admin")) return forbid(); if (String(b.text).length < 20) return err("text", "고지 문구가 너무 짧아요."); S.disclosure = { text: String(b.text), updatedAt: iso(Date.now()), updatedBy: me().name }; S.audit.unshift({ id: S.nextId++, action: "ops.disclosure.update", actor_type: "operator", actor_id: me().id, tenant_id: null, risk_level: "high", created_at: iso(Date.now()), detail: "§16B 고지 문구 변경" }); return { ok: true, ...S.disclosure }; } return { ok: true, ...S.disclosure }; },
    /* ── 공지 ── */
    "ops-notices": (b) => { if (b && b.action) { if (need("admin")) return forbid(); if (b.action === "save") { const n = S.notices.find((x) => x.id === Number(b.id)); if (n) Object.assign(n, b.notice); else S.notices.unshift({ id: S.nextId++, active: true, ...b.notice }); } if (b.action === "toggle") { const n = S.notices.find((x) => x.id === Number(b.id)); if (n) n.active = !n.active; } if (b.action === "delete") S.notices = S.notices.filter((x) => x.id !== Number(b.id)); return { ok: true }; } return { ok: true, notices: S.notices }; },
    /* ── 운영진·감사 ── */
    "ops-operators": (b) => { if (need("super_admin")) return forbid(); if (b && b.action) { if (b.action === "invite") { if (!/@/.test(b.email || "")) return err("email", "이메일을 확인해 주세요."); S.operators.push({ id: S.nextId++, email: b.email, name: b.name || b.email.split("@")[0], role: b.role || "operator", active: true }); } return { ok: true }; } return { ok: true, operators: S.operators }; },
    "ops-operators-role": (b) => { if (need("super_admin")) return forbid(); const o = S.operators.find((x) => x.id === Number(b.id)); if (!o) return err("id", "운영자가 없어요.", { status: 404 }); if (o.id === me().id) return err("self", "내 역할은 바꿀 수 없어요."); o.role = b.role; return { ok: true }; },
    "ops-operators-disable": (b) => { if (need("super_admin")) return forbid(); const o = S.operators.find((x) => x.id === Number(b.id)); if (!o) return err("id", "운영자가 없어요.", { status: 404 }); if (o.id === me().id) return err("self", "나를 끌 수는 없어요."); o.active = !o.active; return { ok: true }; },
    "ops-audit": (_b, q) => { if (need("super_admin")) return forbid(); const s = (q.get("q") || "").toLowerCase(); const t = q.get("tenant"); const a = q.get("actor"); const rows = S.audit.filter((r) => (!s || `${r.action} ${r.detail || ""}`.toLowerCase().includes(s)) && (!t || String(r.tenant_id) === t) && (!a || r.actor_type === a)); return { ok: true, rows: rows.slice(0, Number(q.get("limit")) || 100), page: 1, total: rows.length }; },
  };
