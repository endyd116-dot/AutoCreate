...
(function () {
  ...
  const R = {
    ...
    "ops-audit-search": (_b, q) => { if (need("super_admin")) return forbid(); const s = (q.get("q") || "").toLowerCase(); const t = Number(q.get("tenant")) || 0, a = Number(q.get("actor")) || 0, act = q.get("action") || "", risk = q.get("risk") || "", from = q.get("from") || "", to = q.get("to") || ""; const page = Math.max(1, Number(q.get("page")) || 1);
      const kstDay = (x) => new Date(new Date(x).getTime() + 9 * 3600e3).toISOString().slice(0, 10);
      const rows = S.audit.filter((r) => (!s || `${r.action} ${r.target || ""}`.toLowerCase().includes(s)) && (!t || r.tenant_id === t) && (!a || r.actor_id === a) && (!act || r.action === act) && (!risk || r.risk_level === risk) && (!from || kstDay(r.created_at) >= from) && (!to || kstDay(r.created_at) <= to));
      return { ok: true, rows: rows.slice((page - 1) * 30, page * 30).map((r) => ({ id: r.id, tenantId: r.tenant_id ?? null, actorType: r.actor_type, actorId: r.actor_id ?? null, action: r.action, target: r.target || null, risk: r.risk_level, detail: r.detail ?? null, ip: null, at: r.created_at })), page, total: rows.length }; },
