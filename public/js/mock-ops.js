...
(function () {
  ...
  /* ═══ [R18 · A · 2026-09-26 · 메인 발주] 집필 계약 · 감성 — `/api/ops-emotion`(·`-save`·`-reset`) 흉내 ═══
     🔴 까닭: `/ops/ai.html` 은 R17 B(AC-258 · ae6aa76)부터 이 문을 부르는데 모의엔 **0곳**이었다 — 그래서 그 화면 단추가
        손이 붙었나를 아무 자도 못 쟀다(verify-hand-on-button ⊘ · «모의가 실제보다 작다» · AC-270·AC-275 결).
     🔴 모양은 `netlify/functions/ops-emotion.ts` **글자 그대로**: GET { profiles[{key,channel,emotionKey,label,hasOverlay,overlay,applied,ignored,effective,base,updatedAt}], keys, cacheSeconds, orphans }
        · save { key, applied, ignored, effective, cacheSeconds } / 400 nothing_applied { ignored, keys } · reset { key, removed, cacheSeconds }
     🔴 **표는 생성한다**(아래 ▼GEN 칸 · scripts/gen-mock-ops-emotion.mts 가 lib/writing-contracts.ts 에서 뽑는다) — **식은 옮긴다**(emoApply = applyOverlay · emoSummary = summaryOf).
        손으로 베낀 표는 서버 표가 바뀐 날부터 같은 모양·다른 값으로 조용히 낡는다(AC-275). */
  /* ▼ GEN:emotion — scripts/gen-mock-ops-emotion.mts 가 lib/writing-contracts.ts 에서 뽑아 쓴다 · 🔴 손으로 고치지 마라(다시 돌리면 덮인다) */
  const EMO_BASE = [{"channel":"naver_blog","emotionKey":"story","label":"네이버 블로그 · 경험담·친근","reader":"내 블로그 이웃 — 같은 동네·같은 또래의 친구 같은 사람","register":"구어 존댓말 «~했어요 / ~더라고요 / ~거든요» · 1인칭 경험","rules":["이웃에게 말하듯 쓴다. «~했어요/~더라고요/~거든요» 종결을 섞고, «~합니다/~입니다» 는 쓰지 않는다.","직접 겪은 장면(언제·어디서·무엇을)으로 시작한다. 실패했던 것·바꿔 본 것·개인 판단(«저는 이게 낫더라고요») 을 넣는다.","구체 숫자(가격 원화·시간·횟수)는 페르소나 사정이나 재료에 있는 것만. 없는 수치를 만들지 않는다.","문장 길이를 섞는다 — 짧은 문장이 30% 이상. 문단은 2~4문장 · 길이 불규칙.","소제목엔 이모지를 0~1개까지 허용(본문엔 0)."],"formats":["story","compare","guide","qna","info"],"formatLabel":{"story":"경험담(계기→해봄→결과→팁)","compare":"비교 후기","guide":"실패담→해결","qna":"Q&A","info":"일상+정보"},"structure":{"story":["hook","image","quote","para","image","image","checklist","para","image","divider","para","image","tip","image","hashtags"],"compare":["hook","image","quote","para","table","image","image","para","checklist","image","divider","image","tip","image","hashtags"],"guide":["hook","image","para","quote","image","para","image","checklist","image","divider","para","image","tip","image","hashtags"],"qna":["hook","image","quote","h3","para","image","h3","para","image","h3","para","image","divider","checklist","image","tip","image","hashtags"],"info":["hook","image","para","quote","image","list","para","image","image","divider","para","image","tip","image","hashtags"]},"visual":["사진 6~10장 + 캡션","인용구(핵심 한 줄)","구분선","체크리스트","소제목","해시태그 5~10","장소/링크 카드"],"visualMin":{"image":1,"hashtags":5},"length":{"min":1500,"max":2500},"titleStyle":"naver","titleExample":"에어프라이어 청소, 3분이면 새것처럼","images":{"min":6,"max":10,"default":6,"style":"photo","aspect":"4:3","captionRate":0.3},"emojiPerParagraph":0},{"channel":"tistory","emotionKey":"info","label":"티스토리 · 정보·정리","reader":"검색으로 들어온 사람 — 답을 빨리, 정확히 얻고 싶은 독자","register":"격식 존댓말 «~합니다 / ~입니다» 를 바탕으로, 권유 «~해 보세요»·질문 «~일까요?» 를 섞는 정리 톤","rules":["바탕은 «~합니다/~입니다» 지만 **열 문장 중 두세 문장은 «~해 보세요»(권유)나 «~일까요?»(질문)** 로 쓴다. 전부 «~합니다» 로 끝나면 기계가 쓴 티가 난다.","🔴 문단 하나는 **2~3문장**이다. 네 문장이 넘으면 문단을 나눈다(모바일에서 벽처럼 보인다).","서론-본론-결론. 첫 문단에서 이 글이 답하는 질문을 한 줄로 짚고 시작한다.","표·체크리스트·요약 박스로 한눈에 보이게 한다. 굵게 강조는 문단당 최대 1곳.","근거 없는 수치·최상급 금지. 조건·예외를 «주의» 로 분리해 적는다.","그래도 사람이 쓴 글이다 — 한 문단은 직접 겪은 사례나 개인 판단으로 쓴다."],"formats":["info","listicle","compare","guide"],"formatLabel":{"info":"정보형(문제→원인→방법→주의)","listicle":"리스트형(N가지)","compare":"비교표형","guide":"가이드형(단계)"},"structure":{"info":["toc","para","h2","para","adsense","image","h2","para","table","h2","para","image","list","h2","adsense","summary","faq"],"listicle":["toc","para","h2","para","adsense","image","h2","list","para","h2","para","image","h2","table","adsense","summary","faq"],"compare":["toc","para","h2","para","adsense","table","image","h2","para","h2","list","image","h2","adsense","summary","faq"],"guide":["toc","para","h2","para","adsense","image","h2","checklist","para","h2","para","image","table","h2","adsense","summary","faq"]},"visual":["목차","H2/H3","표","요약 박스","굵게 강조","이미지 2~4장","애드센스 자리 2곳(첫 H2 뒤·마지막 H2 앞)","FAQ"],"visualMin":{"h2":3,"adsense":2,"image":1},"length":{"min":1800,"max":3000},"titleStyle":"google","titleExample":"2026 에어프라이어 청소 방법 총정리","images":{"min":2,"max":4,"default":3,"style":"photo","aspect":"16:9","captionRate":0.2},"emojiPerParagraph":0},{"channel":"blogger","emotionKey":"seo","label":"블로거 · SEO 정보","reader":"구글 검색·AI 답변엔진으로 들어온 사람","register":"격식 존댓말 «~합니다» · 간결한 SEO 정보","rules":["티스토리와 같은 정보 톤. 바탕은 «~합니다» 지만 **열 문장 중 두세 문장은 «~해 보세요»·«~일까요?»** 로 섞는다(전부 «~합니다» 면 기계 티가 난다).","🔴 문단 하나는 **2~3문장**. 네 문장이 넘으면 나눈다.","첫 문단 150자 안에 핵심 답을 준다(메타 설명으로 그대로 쓰인다).","FAQ 3문답(질문은 실제 검색 문장)으로 마무리한다.","이미지 alt 는 장면 설명으로 서로 다르게.","근거 없는 수치·최상급 금지."],"formats":["info","guide","listicle"],"formatLabel":{"info":"정보형","guide":"가이드형","listicle":"리스트형"},"structure":{"info":["para","h2","para","image","h2","table","para","h2","para","image","summary","faq"],"guide":["para","h2","checklist","para","image","h2","para","h2","para","image","summary","faq"],"listicle":["para","h2","list","para","image","h2","para","h2","table","image","summary","faq"]},"visual":["H2/H3","표","FAQ(AEO)","이미지 alt"],"visualMin":{"h2":3,"image":1},"length":{"min":1200,"max":2000},"titleStyle":"google","titleExample":"2026 에어프라이어 청소 방법 총정리","images":{"min":1,"max":3,"default":2,"style":"photo","aspect":"16:9","captionRate":0.5},"emojiPerParagraph":0},{"channel":"threads","emotionKey":"hook","label":"쓰레드 · 훅·짧게","reader":"피드를 훑는 친구 — 첫 줄에서 멈추지 않으면 못 읽는다","register":"친구에게 툭 «~임 / ~함 / ~했는데» 반말 또는 가벼운 존댓말","rules":["첫 줄이 훅이다 — 결론·반전·질문 중 하나로 시작한다.","본문은 3~5줄. 줄바꿈으로 리듬을 만들고 한 줄은 40자 이내.","링크는 본문에 넣지 않는다(댓글에 단다) — 다만 대가 고지는 본문 첫 줄이다. 토픽 태그는 0~1개(쓰레드는 게시물당 1개만 붙는다).","«~에 대해 알아보겠습니다» 류 도입 금지. 바로 본론."],"formats":["story","info","qna"],"formatLabel":{"story":"경험 한 토막","info":"팁 한 줄","qna":"질문→답"},"structure":{"story":["hook","para","para","image"],"info":["hook","list","para","image"],"qna":["hook","para","para","image"]},"visual":["줄바꿈 리듬","이미지 1장"],"visualMin":{"image":1},"length":{"min":120,"max":500},"titleStyle":"hook","titleExample":"에어프라이어 3분 청소법, 진짜 됨","images":{"min":0,"max":1,"default":1,"style":"photo","aspect":"1:1","captionRate":0},"emojiPerParagraph":1},{"channel":"daangn","emotionKey":"hook","label":"당근 새소식 · 동네 말투","reader":"우리 동네 사람 — 가게가 가까운지, 지금 가도 되는지를 먼저 본다","register":"가게 주인이 이웃에게 «~해요 / ~하고 있어요» 나긋한 존댓말","rules":["첫 줄에 **무슨 소식인지**를 그대로 쓴다(«새로 들어왔어요» «이번 주만» 같은 사실 한 줄).","동네·거리·시간을 말한다 — 읽는 사람이 «걸어갈 만한가»를 바로 알 수 있게.","굵게·밑줄·형광펜을 쓸 수 없는 곳이다 — 강조는 줄바꿈과 낱말 순서로만 한다. 별표(**)·마크다운을 본문에 쓰지 않는다.","«~에 대해 알아보겠습니다» 류 도입 금지. 과장·최상급 금지(동네 장사에서 제일 빨리 신뢰를 깎는다)."],"formats":["story","info"],"formatLabel":{"story":"오늘 있었던 일","info":"알아 두면 좋은 것"},"structure":{"story":["hook","para","para","image"],"info":["hook","list","para","image"]},"visual":["사진 1~10장(첫 장이 대표)","줄바꿈 리듬"],"visualMin":{"image":1},"length":{"min":150,"max":700},"titleStyle":"hook","titleExample":"오늘 들어온 딸기, 한 상자 8천원이에요","images":{"min":1,"max":10,"default":3,"style":"photo","aspect":"1:1","captionRate":0},"emojiPerParagraph":0},{"channel":"instagram","emotionKey":"cardnews","label":"인스타 카드뉴스 · 짧고 단정","reader":"썸네일을 넘기며 보는 사람 — 카드 한 장에 한 메시지","register":"짧고 단정한 명사형·구어 «~하기 / ~해요»","rules":["🔴 **카드 한 장 = image 블록 하나**다. 카드마다 `caption` 을 단다 — 그 한 줄이 카드 위에 얹히는 글자다(30자 이내).","🔴 **첫 카드는 표지**(제목을 그대로 베끼지 말고 한 번 더 좁힌다) · **마지막 카드는 행동 유도**(저장·다음 글·프로필). 가운데는 핵심 하나씩.","한 카드에 한 메시지. 두 가지를 한 카드에 넣지 않는다. 숫자·단계로 구조를 준다.","🔴 카드 글자는 **장면 설명이 아니다** — «~하는 모습» «~이 놓여 있는» 금지. 읽는 사람이 바로 가져갈 한 마디를 쓴다.","게시물 본문(캡션)은 2~3줄이면 충분하다 — 카드에서 이미 다 말했다. 해시태그 5~10개."],"formats":["cardnews","steps","listicle","compare","qna"],"formatLabel":{"cardnews":"카드뉴스(표지→핵심→CTA)","steps":"단계형(1단계씩 한 카드)","listicle":"N가지형(하나씩 넘기며)","compare":"비교형(A vs B 표)","qna":"문답형(궁금한 것부터)"},"structure":{"cardnews":["hook","image","image","image","image","image","image","tip","para","hashtags"],"steps":["hook","image","checklist","image","image","image","image","image","summary","hashtags"],"listicle":["hook","list","image","image","image","image","image","image","tip","hashtags"],"compare":["hook","image","table","image","image","image","image","image","quote","hashtags"],"qna":["hook","image","image","image","image","image","image","faq","para","hashtags"]},"visual":["카드 6~8장","카드마다 30자 이내 한 줄","표지 카드","마지막 카드는 행동 유도"],"visualMin":{"image":6,"hashtags":5},"length":{"min":150,"max":400},"titleStyle":"card","titleExample":"에어프라이어 청소 3단계","images":{"min":6,"max":8,"default":6,"style":"infographic","aspect":"1:1","captionRate":0},"emojiPerParagraph":1},{"channel":"wordpress","emotionKey":"seo","label":"워드프레스 · SEO 정보","reader":"구글 검색·AI 답변엔진으로 들어온 사람","register":"격식 존댓말 «~합니다» · 간결한 SEO 정보 · 영문 slug·메타","rules":["티스토리와 같은 정보 톤. 바탕은 «~합니다» 지만 **열 문장 중 두세 문장은 «~해 보세요»·«~일까요?»** 로 섞는다.","🔴 문단 하나는 **2~3문장**. 네 문장이 넘으면 나눈다.","첫 문단 150자 안에 핵심 답(메타 설명).","FAQ 3문답으로 마무리. 이미지 alt 는 장면 설명.","근거 없는 수치·최상급 금지."],"formats":["info","guide","listicle"],"formatLabel":{"info":"정보형","guide":"가이드형","listicle":"리스트형"},"structure":{"info":["para","h2","para","image","h2","table","para","h2","para","image","summary","faq"],"guide":["para","h2","checklist","para","image","h2","para","h2","para","image","summary","faq"],"listicle":["para","h2","list","para","image","h2","para","h2","table","image","summary","faq"]},"visual":["H2/H3","표","FAQ(AEO)","이미지 alt","영문 slug"],"visualMin":{"h2":3,"image":1},"length":{"min":1200,"max":2000},"titleStyle":"google","titleExample":"2026 에어프라이어 청소 방법 총정리","images":{"min":1,"max":3,"default":2,"style":"photo","aspect":"16:9","captionRate":0.5},"emojiPerParagraph":0},{"channel":"youtube_shorts","emotionKey":"script","label":"유튜브 쇼츠 · 3초 훅·자막 본체","reader":"무음으로 스크롤하는 시청자 — 3초 안에 멈추게 해야 한다","register":"구어체 반말/존댓말 · 자막이 본체","rules":["3초 훅(반전·문제 제기·숫자 · ≤16음절 · 도입어 금지) · 문장 4~10개 · 한 문장 ≤ 28음절.","무음 시청 전제 — 자막만으로 이해되게(나레이션은 보조).","컷 4~8(60초는 6~12) · 문장 경계 = 컷 경계 · 엔드카드 1 · 마무리는 행동 한 줄(광고성 CTA 금지).","제휴·유료면 시작 3초 자막 + 우상단 상시 배지 «광고 포함 · 파트너스 수수료» + 설명란 첫 줄 공식 문구(§16B)."],"formats":["story","info","listicle","compare"],"formatLabel":{"story":"반전","info":"문제해결","listicle":"목록(3가지)","compare":"비포애프터"},"structure":{"story":["hook","para","para","para","tip"],"info":["hook","para","para","para","tip"],"listicle":["hook","list","para","tip"],"compare":["hook","para","para","tip"]},"visual":["자막 본체(문장 4~10)","컷 4~8","엔드카드","우상단 배지(제휴 시)","키워드 중앙 자막"],"visualMin":{},"length":{"min":60,"max":235},"titleStyle":"script","titleExample":"에어프라이어, 3분 만에 새것 되는 법","images":{"min":0,"max":0,"default":0,"style":"photo","aspect":"9:16","captionRate":0},"emojiPerParagraph":0},{"channel":"naver_clip","emotionKey":"script","label":"네이버 클립 · 생활밀착·네이버 톤","reader":"네이버 앱에서 생활 정보를 훑는 사람 — 15~30초 · 상단 자막 크게","register":"생활밀착 네이버 톤 · 존댓말 · 짧은 문장","rules":["3초 훅(반전·문제 제기·숫자 · ≤16음절 · 도입어 금지) · 문장 4~10개 · 한 문장 ≤ 28음절.","무음 시청 전제 — 자막만으로 이해되게(나레이션은 보조).","컷 4~8(60초는 6~12) · 문장 경계 = 컷 경계 · 엔드카드 1 · 마무리는 행동 한 줄(광고성 CTA 금지).","제휴·유료면 시작 3초 자막 + 우상단 상시 배지 «광고 포함 · 파트너스 수수료» + 설명란 첫 줄 공식 문구(§16B).","쇼핑커넥트 태그는 메타로 · 본문에서 팔지 않는다."],"formats":["story","info","listicle","compare"],"formatLabel":{"story":"반전","info":"문제해결","listicle":"목록(3가지)","compare":"비포애프터"},"structure":{"story":["hook","para","para","para","tip"],"info":["hook","para","para","para","tip"],"listicle":["hook","list","para","tip"],"compare":["hook","para","para","tip"]},"visual":["자막 본체(문장 4~10)","컷 4~8","엔드카드","우상단 배지(제휴 시)","상단 큰 자막"],"visualMin":{},"length":{"min":60,"max":235},"titleStyle":"script","titleExample":"에어프라이어, 3분 만에 새것 되는 법","images":{"min":0,"max":0,"default":0,"style":"photo","aspect":"9:16","captionRate":0},"emojiPerParagraph":0},{"channel":"reels","emotionKey":"script","label":"릴스 · 감성 b-roll","reader":"감성 피드를 넘기는 사람 — 텍스트 오버레이 · BGM 무드","register":"구어체 반말/존댓말 · 자막이 본체","rules":["3초 훅(반전·문제 제기·숫자 · ≤16음절 · 도입어 금지) · 문장 4~10개 · 한 문장 ≤ 28음절.","무음 시청 전제 — 자막만으로 이해되게(나레이션은 보조).","컷 4~8(60초는 6~12) · 문장 경계 = 컷 경계 · 엔드카드 1 · 마무리는 행동 한 줄(광고성 CTA 금지).","제휴·유료면 시작 3초 자막 + 우상단 상시 배지 «광고 포함 · 파트너스 수수료» + 설명란 첫 줄 공식 문구(§16B).","텍스트 오버레이 큰 글씨 · BGM 무드(라이선스 확인분만)."],"formats":["story","info","listicle","compare"],"formatLabel":{"story":"반전","info":"문제해결","listicle":"목록(3가지)","compare":"비포애프터"},"structure":{"story":["hook","para","para","para","tip"],"info":["hook","para","para","para","tip"],"listicle":["hook","list","para","tip"],"compare":["hook","para","para","tip"]},"visual":["자막 본체(문장 4~10)","컷 4~8","엔드카드","우상단 배지(제휴 시)","키워드 중앙 자막"],"visualMin":{},"length":{"min":60,"max":235},"titleStyle":"script","titleExample":"에어프라이어, 3분 만에 새것 되는 법","images":{"min":0,"max":0,"default":0,"style":"photo","aspect":"9:16","captionRate":0},"emojiPerParagraph":0},{"channel":"tiktok","emotionKey":"script","label":"틱톡 · 3초 훅","reader":"무음으로 스크롤하는 시청자 — 3초 안에 멈추게 해야 한다","register":"구어체 반말/존댓말 · 자막이 본체","rules":["3초 훅(반전·문제 제기·숫자 · ≤16음절 · 도입어 금지) · 문장 4~10개 · 한 문장 ≤ 28음절.","무음 시청 전제 — 자막만으로 이해되게(나레이션은 보조).","컷 4~8(60초는 6~12) · 문장 경계 = 컷 경계 · 엔드카드 1 · 마무리는 행동 한 줄(광고성 CTA 금지).","제휴·유료면 시작 3초 자막 + 우상단 상시 배지 «광고 포함 · 파트너스 수수료» + 설명란 첫 줄 공식 문구(§16B)."],"formats":["story","info","listicle","compare"],"formatLabel":{"story":"반전","info":"문제해결","listicle":"목록(3가지)","compare":"비포애프터"},"structure":{"story":["hook","para","para","para","tip"],"info":["hook","para","para","para","tip"],"listicle":["hook","list","para","tip"],"compare":["hook","para","para","tip"]},"visual":["자막 본체(문장 4~10)","컷 4~8","엔드카드","우상단 배지(제휴 시)","키워드 중앙 자막"],"visualMin":{},"length":{"min":60,"max":235},"titleStyle":"script","titleExample":"에어프라이어, 3분 만에 새것 되는 법","images":{"min":0,"max":0,"default":0,"style":"photo","aspect":"9:16","captionRate":0},"emojiPerParagraph":0}];
  const EMO_KEYS = ["label","reader","register","rules","formats","formatLabel","structure","visual","visualMin","length","titleStyle","titleExample","images","emojiPerParagraph"];
  const EMO_CACHE_SECONDS = 60;
  /* ▲ GEN:emotion */
  /* 🔴 라이브 `emotion_profiles` = **3행**(메인 실측 2026-09-26) — 값은 scripts/seed-plans.mjs:58~60 시드 그대로(짧은 옛 모양 · tone·배열 length/images).
     유령 행 `youtube_shorts.graphic` 은 2026-09-26 메인이 지웠다 — **넣지 마라**(넣으면 orphans 가 1 이 된다 · 라이브는 빈 배열).
     🔴 이 짧은 모양이 서버 applyOverlay 를 지나면 **안 먹는 칸이 실제로 생긴다**(structure 배열 ↔ 표는 객체 등) — 그게 라이브의 «안 먹은 칸» 모양이다. */
  const EMO_LIVE = {
    "naver_blog.story": { tone: "구어 존댓말(~했어요/~더라고요)", structure: ["hook", "photo+caption", "quote", "body", "photos", "checklist", "body", "divider", "tip", "hashtags"], images: [6, 10], length: [1500, 2500], titleStyle: "naver" },
    "tistory.info": { tone: "격식 존댓말(~합니다)", structure: ["toc", "h2", "table|list", "h2", "adsense", "h2", "summary", "faq", "adsense"], images: [2, 4], length: [1800, 3000], titleStyle: "google" },
    "threads.hook": { tone: "가벼운 반말/존댓말", structure: ["hook", "3-5lines", "link-in-reply"], length: [0, 500] },
  };
  /* updated_at — 라이브 3행 **모두** 2026-09-13T10:43:11(UTC 저장 · 메인이 라이브에서 읽음) = 시드 뒤로 아무도 안 바꿨다. `tone` 은 그대로 둔다(라이브 모양 · 서버가 register 로 옮겨 먹는다). */
  if (!S.emotion) { S.emotion = Object.fromEntries(Object.entries(EMO_LIVE).map(([k, c]) => [k, { contract: c, updatedAt: "2026-09-13T10:43:11.000Z" }])); save(); }
  const emoKeyOf = (c) => `${c.channel}.${c.emotionKey}`;   // contractFor 와 같은 열쇠 규칙
  const emoShapeOf = (x) => (Array.isArray(x) ? "array" : x === null ? "null" : typeof x);
  /** = lib/writing-contracts.ts applyOverlay (식째로 옮김 · 바꾸면 거기와 같이) */
  function emoApply(base, o) {
    const merged = { ...base }; const applied = [], ignored = [];
    ...
    const known = new Set(EMO_KEYS);
    for (const k of Object.keys(o2)) { if (k === "tone") continue; if (!known.has(k)) { ignored.push({ key: k, why: "계약에 없는 칸이에요" }); continue; } }
    for (const k of EMO_KEYS) {
      const v = o2[k]; if (v === undefined || v === null) continue; const cur = base[k];
      if (emoShapeOf(cur) !== emoShapeOf(v)) { ignored.push({ key: k, why: "모양이 달라요", expected: emoShapeOf(cur), got: emoShapeOf(v) }); continue; }
      if (emoShapeOf(cur) === "object") merged[k] = { ...cur, ...v }; else merged[k] = v;
      applied.push(k);
    }
    return { merged, applied, ignored };
  }
  /** = netlify/functions/ops-emotion.ts summaryOf (화면이 그대로 그리는 요약 넉 줄) */
  const emoSummary = (c) => [
  ...
  const R = {
    /* [R18 · A] 집필 계약 · 감성 — 조회 operator+ · 변경 super_admin(서버 requireAdmin 과 같은 갈래) */
    "ops-emotion": () => {
      const profiles = EMO_BASE.map((base) => { const key = emoKeyOf(base); const row = S.emotion[key];
        const overlay = row && row.contract && typeof row.contract === "object" ? row.contract : null; const rep = emoApply(base, overlay);
        return { key, channel: base.channel, emotionKey: base.emotionKey, label: base.label, hasOverlay: !!overlay && Object.keys(overlay).length > 0, overlay: overlay ?? {},
          applied: rep.applied, ignored: rep.ignored, effective: emoSummary(rep.merged), base: emoSummary(base), updatedAt: row ? row.updatedAt : null }; });
      const known = new Set(profiles.map((p) => p.key));
      return { ok: true, profiles, keys: EMO_KEYS, cacheSeconds: EMO_CACHE_SECONDS, orphans: Object.keys(S.emotion).filter((k) => !known.has(k)) }; },
    "ops-emotion-save": (b) => { if (need("super_admin")) return forbid();
      const key = String(b.key ?? "").trim(); const base = EMO_BASE.find((c) => emoKeyOf(c) === key); if (!base) return err("key", "그런 집필 계약이 없어요.");
      const overlay = b.overlay; if (!overlay || typeof overlay !== "object" || Array.isArray(overlay)) return err("overlay", "바꿀 내용을 객체로 보내 주세요.");
      const rep = emoApply(base, overlay);
      if (!rep.applied.length) return { ok: false, step: "nothing_applied", error: "바뀌는 칸이 하나도 없어요. 아래에서 까닭을 확인해 주세요.", ignored: rep.ignored, keys: EMO_KEYS, status: 400 };
      S.emotion[key] = { contract: JSON.parse(JSON.stringify(overlay)), updatedAt: iso(Date.now()) };
      return { ok: true, key, applied: rep.applied, ignored: rep.ignored, effective: emoSummary(rep.merged), cacheSeconds: EMO_CACHE_SECONDS }; },
    "ops-emotion-reset": (b) => { if (need("super_admin")) return forbid();
      const key = String(b.key ?? "").trim(); if (!EMO_BASE.some((c) => emoKeyOf(c) === key)) return err("key", "그런 집필 계약이 없어요.");
      const removed = !!S.emotion[key]; delete S.emotion[key];
      return { ok: true, key, removed, cacheSeconds: EMO_CACHE_SECONDS }; },
  ...
  const badge = document.createElement("div"); badge.textContent = `모의 데이터 · ${role}`; badge.style.cssText = "position:fixed;bottom:calc(var(--tab-h) + 6px);left:8px;z-index:99;font-size:10px;font-weight:700;color:var(--surface);background:var(--ink);border-radius:6px;padding:2px 6px;pointer-events:none"; document.body.appendChild(badge);
})();
