...
<p>정리하면, 주 1회 10분이면 냄새와 연기 없이 쓸 수 있다.</p>`;
  ...
  const HOOKS = ["contrast", "question", "number_typo", "event_pushin", "extreme_closeup"], PALETTES = ["ink", "coral", "mint", "sand", "night"]; // B-1 types.ts 훅 5종 · 팔레트 5
  ...
  const videoSpec = (i, accountId, seconds = 60, format = "graphic") => ({ format, seconds, provider: { tier: seconds <= 15 ? "filler" : "standard", key: seconds <= 15 ? "veo_lite" : "omni" }, voice: { provider: "typecast", voiceId: VOICES[i % VOICES.length].voiceId },
    variant: { hookType: HOOKS[i % HOOKS.length], palette: PALETTES[(Number(accountId || 0) + i) % PALETTES.length], voiceId: VOICES[i % VOICES.length].voiceId }, cuts: format === "clip" ? 3 : seconds >= 60 ? 9 : 5, disclosure: { badge: true, descriptionFirstLine: true } });
