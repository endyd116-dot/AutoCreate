...
export async function run({ ctx, job, shotKey }) {
  ...
  try {
    ...
    const parsed = rowsToRevenue("adfit", table.rows.map((r) => ({ dayText: r[table.dayIdx], amountText: r[table.amountIdx], raw: { cells: r.slice(0, 6), ...(table.amountEstimated ? { estimated: true } : {}) } })), { accountId: account.id });
