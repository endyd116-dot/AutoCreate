...
export async function run({ ctx, job, shotKey }) {
  ...
  try {
    ...
    const parsed = rowsToRevenue("adfit", table.rows.map((r) => ({ dayText: r[table.dayIdx], amountText: r[table.amountIdx], raw: { cells: r.slice(0, 6) } })), { accountId: account.id });
    if (!parsed.ok) throw PARSE(`${parsed.reason} · 머리글=${JSON.stringify(table.header).slice(0, 80)}`);
    return { revenueRows: parsed.rows, notes: [`수입 ${parsed.rows.length}행(${table.where})`] };
  } catch (e) {
