/**
 * cron-tick-hourly — 정시 우산(계약 §1 · DESIGN §5B.7). **Netlify 스케줄 전용 함수.**
 ...
 *   ⇒ **수동 강제 실행·검증은 `/api/cron-run?every=hourly&secret=$CRON_SECRET`**(`netlify/functions/cron-run.ts`).
...
export default async (req: Request): Promise<Response> => {
  ...
  catch (err) { console.error("[cron-tick-hourly]", err); return new Response(JSON.stringify({ ok: false, step: "cron_tick_hourly" }), { status: 500 }); }
};
