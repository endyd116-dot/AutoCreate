...
import { aiStubActive } from "../../lib/ai-stub";   // [R8 §2.1] 글 실호출 대체 스위치가 **이 런타임에** 켜졌나
...
export default async (): Promise<Response> => {
  try {
    ...
    return json({ ok: true, db: r[0]?.ok === 1, ts: new Date().toISOString(), deploy, commit, dev: isNetlifyDev(), loadedAt: LOADED_AT, stub: videoStub(), aiStub: aiStubActive() });
