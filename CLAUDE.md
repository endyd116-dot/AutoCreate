...
- 🔴 **겁주지 않는다(사장님 지시 2026-09-15)**: «앞으로는 우리가 **알려만 드리고** 그대로 올라가요» 같은 **협박성·면피성 문장 금지.**
  같은 뜻을 **우리 편에서** 말한다 → «**주요 안내는 사전에 알려드려요.**»
  - **금지**: 위협(«정지됩니다» «불이익이 있습니다») · 책임 전가(«고객님 책임입니다» «알려만 드렸습니다») · 겁주는 조건절(«~하면 ~될 수 있습니다»로 끝내기).
  - **한다**: ①**사실 한 줄**(무엇이 그런가) ②**어떻게 하면 되는지** ③**우리가 대신 해 주는 것**. 위험은 **재료**로 주고 **고르는 것은 고객**이다.
  - 🔴 **§9(게이트 0개)와 한 몸이다** — 막지 않는 대신 말해 주기로 갔는데 그 말투가 겁주는 것이면 **게이트보다 나쁘다**(막지도 않으면서 불안만 준다).
...
> 🔴 **최종(사장님 2026-09-15 · 같은 날 세 번째)**: **«말해 주기로 내려. 고객 계정이야. 우리가 책임지는 게 아니야.»**
> ⇒ **하드 게이트는 0개다.** 내가 두 번에 걸쳐 «법은 남기자 · 제3자는 남기자»고 물었고 사장님이 두 번 다 **내리라**고 하셨다.

## 하드(막는다) = **없다**

**모든 검사는 소프트다.** 검사는 **계속 돌고 결과를 보여 준다.** 다만 **예약·발행을 막지 않는다.**
대가 고지 · 금칙 · 의료 경험담 · 스톡 사람/상표 · 광고 유도 · 제휴 링크 수 · 유사도 · 품질 · 모양 · 분량 · 반복 · 빈도 · 골격 · 시각 요소 최소치 — **전부.**

근거: **그 계정은 고객 것이고, 우리는 도구다.** 플랫폼 정책은 법도 아니고 수시로 바뀌며, 우리가 고객 대신 지킬 의무가 없다. 법 해석도 **우리가 고객 대신 할 일이 아니다.**

### 그래서 «말해 주기»가 더 세져야 한다 — 이게 이 규칙의 값이다

막지 않는 대신 **아래 넷을 반드시 지킨다.** 막는 것보다 이게 어렵다.
1. 🔴 **또렷하게 말한다** — 위험한 것은 «지나가는 회색 글씨»가 되면 안 된다. **무엇이 · 왜 · 어떻게 하면 되는지** 세 줄.
   🔴 **또렷하게 ≠ 겁주기**(사장님 2026-09-15). «알려만 드립니다»로 발을 빼거나 «정지됩니다»로 위협하지 않는다 — **«주요 안내는 사전에 알려드려요»** 쪽이다(§3 말투 규칙).
2. 🔴 **사람이 안 보는 경로(자동 승인)에서도 닿게 한다** — 자동으로 나가면 «말해 주기»가 아무에게도 안 닿는다. ⇒ **알림 + 발행 뒤에도 «이 글엔 이런 위험이 있었어요»를 볼 수 있게** + 자동 승인을 **켤 때** 무엇을 켜는지 알린다.
3. 🔴 **되돌릴 길을 함께 준다** — 내리기(§5E)가 있으니 «나갔어도 되돌릴 수 있다»가 성립한다. **말해 주기는 되돌릴 길이 있어야 정직하다.**
4. 🔴 **우리가 대신 해 줄 수 있는 것은 대신 해 준다** — 고지 문장은 **막는 게 아니라 넣어 주는 것**이다(`ensureDisclosureHtml`). 켜면 자동으로 첫머리에 실린다. **막지 말라는 것이지 빼라는 것이 아니다.**

### 이 규칙 **밖**인 것 (게이트가 아니다)
- **돈·계약** — 플랜 한도 · 잔액 부족 · 체험 종료 readonly · 결제 밀림 · 탈퇴. 우리 판단이 아니라 **계약**이다.
- **없는 길** — 커넥터 미연결 · `publishVia: null` · 유튜브 못 내림. 게이트가 아니라 **사실**이다. §9 는 «없는 길을 열라»가 아니라 **«있는 길을 우리 판단으로 막지 말라»**이다.
- **되돌릴 수 없는 동작의 확인 대화** — «정말 내릴까요?» 는 고객을 위한 확인이지 우리 판단이 아니다.

### 그 밖
- 🔴 **검사를 지우지 마라.** 소프트로 내리는 것이지 **없애는 것이 아니다** — 말해 주려면 재야 한다.
- 🔴 **계약의 «최소치»도 게이트다**(`visualMin` 처럼 «몇 개 이상»). 계약의 다른 부분과 싸우면 **매번 재작성이 돌아 돈이 두 배**다(2026-09-15 blogger·wordpress `faq` 실제 사례).
- 🔴 **«아직 모른다»로 막지 않는다** — 표본이 없어 못 재는 축은 통과시키고 «못 쟀어요»라고 적는다(AC-9·AC-60).
- 화면은 «막지 않아요»를 함께 말한다(`UI.GATE_HARD` ↔ 서버 `HARD_GATE_KEYS` · 하니스 대조 — **이제 서버 목록이 빈 배열이다**).
