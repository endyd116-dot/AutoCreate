...
**하드(막는다)로 둘 수 있는 것은 둘뿐이다.**
1. **법·플랫폼 정책 위반** — 대가 고지(`disclosure`) · 금칙 HARD 층(`banned_words`) · 광고를 가리키는 유도(`ad_pointing`) · 스톡 사람·상표(`stock_safe`) · 의료 경험담.
2. **되돌릴 수 없는 것** — 한 번 나가면 못 주워 담는 것.

**나머지는 전부 소프트(알려 주기만)다.** 품질 · 모양 · 분량 · 반복 · 빈도 · 골격 · 시각 요소 최소치 · 링크 열림 — 막지 않는다.

- 🔴 **막으면 공장이 선다.** 고객이 «알아서 올려»를 켰는데 우리가 조용히 안 올리면 **그게 더 나쁘다.** 고객이 못 고치는 것 앞에서 멈춰 세우는 게이트는 기능이 아니라 고장이다.
- 🔴 **게이트를 새로 만들 때는 «하드인가»를 먼저 묻고, 기본값은 소프트다.** 하드로 하려면 위 1·2 중 어디에 해당하는지 **한 줄로 적는다.** 못 적으면 소프트다.
- 🔴 **계약의 «최소치»도 게이트다.** `visualMin` 처럼 «몇 개 이상»을 요구하는 값은 **숨은 하드 게이트**다 — 그 값이 계약의 다른 부분(`tiers.optional`)과 싸우면 **매번 재작성이 돌아 돈이 두 배로 든다**(2026-09-15 blogger·wordpress `faq` 실제 사례).
- 🔴 **막는 대신 말한다.** 위험은 고객에게 **문장으로** 주고 고르게 한다(간격·새벽·새 계정). 우리가 대신 정하지 않는다.
- 🔴 **«아직 모른다»로 막지 않는다.** 표본이 없어 판정을 못 하는 축은 **통과**시키고 «못 쟀어요»라고 적는다(AC-9·AC-60). 못 잰 것을 «실패»로 세면 첫 고객이 글을 못 낸다.
- 소프트 축은 화면이 **«막지 않아요»**를 함께 말한다(A `UI.GATE_HARD` 가 서버 `HARD_GATE_KEYS` 를 복사하고 하니스가 대조한다).
