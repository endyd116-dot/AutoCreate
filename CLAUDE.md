...
- 🔴 **DDL 번호는 세션마다 칸이 정해져 있다(2026-09-15 메인 결정 · B-1 제안)** — `ls drizzle/ | tail -1` 로는 **못 막는다**(그 사이 다른 창이 머지된다 · 하루에 **여덟 번** 겹쳤다).
  🔴 **칸의 정본은 [`docs/active/LANES.md`](docs/active/LANES.md) 다.** 판마다 메인이 다시 나누므로 **여기 표를 두지 않는다** —
  두면 곧 낡고, 낡은 표를 믿은 창이 **남의 칸을 밟는다**(2026-09-23 에 겹친 번호 셋이 전부 그렇게 났다 · AC-175 ④).
  · 시작할 때 `LANES.md` 를 읽는다. **새 트리거가 칸을 말하면 트리거가 우선**이다.
  · **자기 칸 안에서는 물어볼 것 없이 자율**이고, 칸이 차면 메인에 한 줄 — 메인이 새 칸을 나누고 **`LANES.md` 를 먼저 고친 뒤** 알린다.
  · 🔴 **남의 칸은 비어 보여도 쓰지 마라** — 비어 보이는 것과 안 쓴 것은 다르다.
