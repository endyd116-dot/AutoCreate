...
export const COIN_TABLE: Record<CoinItem, number> = {
  /* [R12-7] `video_90` = **42**(60초 28 × 1.5).
     ✅ **2026-09-21 사장님 결재 — 확정값이다.** (전에 «B 가 잡은 임시값 · 결재 대기»였다. 🔴 주석이 코드보다 뒤처지면
        다음 사람이 또 «대기»로 읽고 손을 못 댄다 · AC-59 — 그래서 결재 난 날 같이 고쳤다.)
     근거: 원가가 컷 수에 붙고 90초 그래픽은 컷이 9→13·나레이션이 1.5배다. 15/30/60 이 초당 0.40·0.40·0.47 코인이라 90 도 같은 결(0.47)로 뒀다.
     결재에 올린 숫자(고친 컷 수로 다시 뽑은 것): 90초 그래픽 **추정** 원가 ₩12,923 · 받는 돈 ₩21,000 · **×1.63**(60초 ×1.56 과 같은 잣대).
     ⚠️ 그 ₩12,923 은 **추정**이다 — 실제 청구서와 맞춘 적이 없다(`docs/active/2026-09-21-video-cost-retry-opinion.md`).
