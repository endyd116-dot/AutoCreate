 ...
 *   주문번호 `AC-BK-{tid}-{base36}`(콜백 되파싱 · 세션 없음).
 */
...
export const bkOrderNo = (tid: number, nowMs = Date.now()) => `AC-BK-${tid}-${nowMs.toString(36)}`;
export function parseBkOrderNo(orderNo: string): number | null { const m = /^AC-BK-(\d{1,12})-[0-9a-z]+$/.exec(String(orderNo ?? "").trim()); return m ? Number(m[1]) : null; }
...
export async function startBillingKey(tid: number, opts: { userAgent?: string | null; returnBase?: string }): Promise<StartKeyResult> {
  ...
  const orderNo = bkOrderNo(tid);
  const base = (opts.returnBase || process.env.SITE_URL || "").replace(/\/$/, "");
  const r = await registerTrade({ shopOrderNo: orderNo, amount: 0, goodsName: "AutoCreate 결제 수단 등록", isBillingKey: true, returnUrl: `${base}/api/billing-key-return`, customerName: owner.name, customerEmail: owner.email ?? undefined, deviceTypeCode: deviceTypeFromUA(opts.userAgent) });
...
export async function approveBillingKey(authorizationId: string, orderNo: string): Promise<ApproveKeyResult> {
  const tid = parseBkOrderNo(orderNo);
  if (!tid) return { ok: false, reason: "bad_order_no" };
  ...
  await q(sql`INSERT INTO billing_keys (tenant_id, billing_key, card_label, active, card_fp, last4, brand) VALUES (${tid}, ${String(a.billKey)}, ${brand && last4 ? `${brand} ${last4}` : last4}, true, ${fp}, ${last4}, ${brand})`);
  ...
  await writeAudit({ tenantId: tid, action: "billing_key_registered", actorType: "system", target: `tenant:${tid}`, detail: { brand, last4, hasFp: !!fp, trialEndedByFp } });
...
export async function removeBillingKeyOf(tid: number, actorId: number | null): Promise<{ ok: boolean; removed: boolean }> {
  const [k] = await q(sql`SELECT id, billing_key FROM billing_keys WHERE tenant_id = ${tid} AND active = true AND removed_at IS NULL ORDER BY id DESC LIMIT 1`);
  if (!k) return { ok: true, removed: false };
  try { const { isKiccConfigured, removeBillingKey } = await import("../kicc"); if (isKiccConfigured()) await removeBillingKey({ billingKey: String(k.billing_key) }); } catch { /* PG 쪽 삭제 실패는 비치명 — 우리 쪽에서 비활성 */ }
