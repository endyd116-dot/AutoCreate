...
export async function requireAiBudget(tid: number): Promise<{ ok: true; check: CapCheck } | { ok: false; check: CapCheck; error: string }> {
  ...
  if (!dup) {
    await q(sql`INSERT INTO notifications (tenant_id, kind, title, body, link) VALUES (${tid}, ${"ai_cost_cap"}, ${"오늘 만들 수 있는 양을 다 썼어요"}, ${`오늘 AI 사용이 하루 상한(${check.capKrw.toLocaleString("ko-KR")}원)에 닿았어요. 내일 다시 이어서 만들어요.`}, ${"/app/home.html"})`);
