 ...
 *
 *   ══ 하드 게이트 — [P1R8 §9] **셋뿐이다**(사장님 전역 지시 2026-09-15) ══
 *     ① 대한민국 법령 위반 ② 제3자가 다치는 것(저작권·초상권) ③ 되돌릴 수 없는 것 → `disclosure` · `banned_words` · `stock_safe`.
 *     🔴 **플랫폼 정책은 하드의 근거가 아니다** — «애드센스·네이버가 싫어한다»만으로는 막지 않는다(정책은 계속 바뀌고, 우리가 그걸로 고객을 제약할 의무도 없다).
 *     나머지(AI 티·구성·길이·유사도·제휴 링크 수·광고 지칭)는 **보여 주되 막지 않는다** — 위험은 문구로 말한다.
 *
