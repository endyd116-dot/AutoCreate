...
export async function generateVideo(tid: number, pieceId: number, opts: { resume?: boolean; by?: string } = {}): Promise<GenResult> {
  ...
  try {
    ...
    if (!opts.resume || !script) {
      ...
      const r = await buildVideoScript({ syllableRatio, tenantId: tid, pieceId, format, seconds, cuts, channel, topic: { title: topic.title, angle: String(meta.angle || topic.angle), intent: topic.factors.intent, seasonal: topic.factors.seasonal }, persona: { facts: persona.facts, tone: persona.tone, signature: persona.signature, dict: persona.dict }, hookType: spec.variant?.hookType ?? "event_pushin", structure: (meta.structure as string[] | undefined) ?? null, hookPrinciple: refStyle?.hookPrinciple ?? null, affiliate: aff ? { productQuery: aff.productQuery } : null, regenNote: String(meta.regenNote ?? "").trim().slice(0, 300) || null });
      ...
      if (!g.ok) {
        const r2 = await buildVideoScript({ syllableRatio, tenantId: tid, pieceId, format, seconds, cuts, channel, topic: { title: topic.title, angle: String(meta.angle || topic.angle), intent: topic.factors.intent, seasonal: topic.factors.seasonal }, persona: { facts: persona.facts, tone: persona.tone, signature: persona.signature, dict: persona.dict }, hookType: spec.variant?.hookType ?? "event_pushin", structure: (meta.structure as string[] | undefined) ?? null, hookPrinciple: refStyle?.hookPrinciple ?? null, affiliate: aff ? { productQuery: aff.productQuery } : null, regenNote: String(meta.regenNote ?? "").trim().slice(0, 300) || null, rewrite: `[다시 쓰기 — 아래에 걸렸다]\n${g.issues.map((x) => `- ${x}`).join("\n")}` });
