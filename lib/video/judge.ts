 ...
 *     러너가 지문을 한 번도 보낸 적이 없어서(2026-09-15 설계감사 19번)
 *     이 축은 **늘 초록이었다**. 게이트가 있는데 아무것도 막지 않는 상태 — 있는 줄 알고 안심하는 게 더 나쁘다.
 *
 *   🔴 ══ [R17-B2 · 2026-09-23 · B 가 «이 주석 낡았다»고 짚어서 다시 쟀다] 반만 낡았다 ══
 *     · **코드 경로는 생겼다** — `runner/channels/render-video.mjs:832` 가 `thumbGray` 를 싣고
 *       `lib/runner-jobs.ts:1799-1800` 이 받는다(길이·모양 검사까지). «전송 0» 은 이제 소스에 대해 **틀린 말**이다.
 *     · 🔴 **그런데 라이브로 온 지문은 여전히 0이다** — `runner_jobs` 에 `render.video` 가 **한 건도 없다**
 *       (2026-09-23 실측: 상태별 0행 · `result->>'fingerprint'` 0행). 렌더 잡이 한 번도 적재·실행된 적이 없다.
 *     ⇒ 🔴 **아래 세 갈래는 그래서 여전히 필요하다.** «코드가 보낸다»를 «지문이 있다»로 읽으면
 *        ③(견줄 상대는 있는데 지문이 없다)을 ②(진짜 통과)로 접게 된다 — 그게 이 주석이 원래 막으려던 그 사고다.
 *     («러너 코드에 있다» ≠ «러너가 실제로 보냈다» · AC-178 대용물.)
 *     이제 셋을 가른다:
 *       ① 지문이 있다        → 실제로 잰다(해밍 거리)
