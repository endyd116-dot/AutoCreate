 ...
 *
 *   ══ 🔴 이 판정으로 **막지 않는다**(사장님 전역 지시 2026-09-15 · CLAUDE §9 «게이트는 최소화») ══
 *     처음엔 자동 승인을 막는 데 썼는데, 라이브 계정 12개가 **전부 «아직»**이라 자동 발행이 통째로 섰다.
 *     «알아서 올려»를 켠 고객의 글이 한 편도 안 나가는 것이 더 나쁘다 ⇒ **알림·감사·운영 숫자로만** 쓴다.
 *     하드로 막아도 되는 것은 ①법·플랫폼 정책 위반 ②되돌릴 수 없는 것 둘뿐이고, «계정이 아직 처음이다»는 그 어느 쪽도 아니다.
 */
