...
### ②-b 🔴 «고객 한 곳만» — **내 것부터 이 자로 다시 봤다**(커밋 `a28259e`)
API 는 `tenantId` 를 받는데 **화면이 아무 데서도 안 보냈다.** 보내는 데가 **내 스모크 하나뿐** —
그게 바로 오늘 자가 잡은 «**시험 대본만 쓰는 칸**»이다. 손잡이를 내고 같은 병을 새로 만들 뻔했다.
⇒ 같은 칸에 «고객 한 곳만 멈추기 · 다시 켜기»를 달았다(멈춰 둔 고객 목록 + 번호로 한 곳 멈추기).
말썽인 고객 하나만 멈추는 것은 전 고객을 멈추는 것보다 **훨씬 자주 쓸 손잡이**다.
...
✓ 🔴 고객 번호 761 로 **한 곳만** 멈추면 «따로 멈춘 고객 1곳» · toast «761번 고객 영상을 멈췄어요»
✓ pageerror 0
```
