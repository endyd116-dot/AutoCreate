...
### 🔴 R12 뒤에 잡힌 것 — **러너 새 판을 올려야 하는 사유가 하나 더 생겼다**(2026-09-17)

> **`eof_action=pass` 탓에 자막·제휴 고지·배지·엔드카드가 «첫 프레임 말고는» 영상에 안 실리고 있었다.**
> C 가 «나온 파일»에서 찾았고 B2 가 ffmpeg 8.1.2 로 실측 확인했다(창 **안** 1.5초 픽셀: `pass` (0,0,0) ↔ `repeat` (252,0,0)).
> 오버레이 입력이 **단일 프레임**이라 t=0 에 EOF 고, `pass` 는 그때부터 본편을 통과시킨다 ⇒ `enable` 창이 와도 안 얹힌다.
> `repeat` 은 overlay 의 **기본값**이다 — 종전 코드는 기본값을 **일부러 껐고 그게 기능을 껐다.**

🔴 **이미 R2 에 올라간 러너(1.3.0)는 여전히 `pass` 다.** 고객에게 가려면 **새 판을 올려야 한다**(사장님 Allow · AC-50).
⇒ `OWNER-CHECKLIST §5.2` 의 «누르기 직전 상태» 문장에 **이 사유를 한 줄 더해야 한다**(제휴 고지가 영상에 안 실리고 있었다 = 법이 읽는 문장).
⚠️ 판 의존일 수 있다 — **잰 판은 8.1.2 하나**다. 옛 판에서 돌았는지는 **모른다**(«전부 고장이었다»고 말하지 않는다).

**여기서 배운 것 둘**
1. 🔴 **인자를 재는 자는 «나온 파일»을 원리상 못 잡는다.** 내 자는 인자까지고 C 는 파일이다 — **한 층만 보면 또 난다.**
2. 🔴 **`framemd5` 로 «전 프레임이 달라졌다»를 판정하지 마라**(C 가 알려 준 것): 프레임 0 하나만 바뀌어도
   뒤의 P 프레임이 그걸 참조해 **전부 다르게 나온다.** 창 «안쪽» 프레임을 **픽셀로** 봐야 한다.

### 🔴 R12 에서 세운 규칙 둘 — **다음 사람이 그대로 쓰라고 적는다**

1. 🔴 **«못 잰다»로 내려앉는 조건에 입력이 둘 이상이면 대조군이 있어야 한다**(2026-09-17 B 가 세운 규칙).
   «그 상태가 뜨나»만 재면 **조건을 넓혀도 초록**이다 — 넓히면 오히려 «늘 뜨는» 쪽이라 더 잘 통과한다.
   내 쪽 적용: `motionForLayer`(kind × windowMs × typeMotion) · `pickVerifyLayer`(층이 있나 × 창이 충분한가) —
   **양쪽을 다** 재고, 변이로 «그 입력을 안 본다»를 심어 빨개지는지 확인했다.
2. 🔴 **«두 판을 견주는» 검사는 «다른 곳이 같은지»를 먼저 증명해야 한다.**
   실측(2026-09-17): 오버레이 층을 **통째로 빼고** 견줬더니 `overlay` 필터가 **사라져**
   **아무것도 안 그려도 바이트가 달라졌다** ⇒ «달라졌으니 그려졌다»가 늘 참이 되어 **옛 `pass` 판을 못 잡았다.**
   ⇒ **빼지 말고 창만 민다**(사슬 모양을 그대로 둔다) · **x264 를 안 태운다**(`-t` 가 다르면 율 제어부터 갈린다).
   ⚠️ **거짓 초록을 고치러 만든 검사가 또 거짓 초록이 될 뻔했다.** «줄을 넣었다»로 끝내지 마라.

### 🔴 R12 배포 뒤에 한 것 — 러너 v1.4.0 이 **실제로 올라갔다**(2026-09-19)

| | |
|---|---|
| 전 | **v1.1.8**(2026-09-15 18:01 KST · 128,685 B) — 🔴 **문서가 «1.3.0» 이라고 세 번 인용했는데 틀렸다**(소스를 본 값) |
| 후 | **v1.4.0**(2026-09-19 02:24 KST · 200,130 B · sha256 `0864a8e84ba10e78…`) |

- 🔴 **v1.2.0·v1.3.0 은 한 번도 안 올라갔었다** ⇒ R8·R9·R10·R12 가 **하나도 안 가 있었다**(격차 19파일 +2,779/−168).
- 읽는 자 둘을 남겼다(**둘 다 읽기만**): `scripts/read-runner-live.mts`(번호·존재·바이트) ·
  🔴 `scripts/verify-runner-live-inside.mts`(**zip 을 받아서 열고 코드 글자를 본다** · `--version=` 으로 아무 판이나).
  **전후가 갈린다**: v1.4.0 → 10/10 초록 · v1.1.8 → 10/10 빨강.
- ⚠️ **판정 문장은 «올라갔다»까지다** — 러너가 자기 주기로 받아 가므로 **«전 고객이 새 판이다»가 아니다.**

### 🔴 여기서 나온 병 하나 — **`docs/rules/PITFALLS.md` AC-109 에 정본으로 넣었다**

> **«세기 쉬운 것»을 «알고 싶은 것» 대신 센다.** 하루에 다섯 번 나왔고 다섯이 **한 가지 병**이었다:
> 주석/본문 · **번호/내용** · 길이/내용 · 프레임 해시/보이는 픽셀 · 엔드포인트 수/영향 범위.

- 잣대를 **한 곳**으로 모았다: `scripts/_lib/code-only.mjs` + 자 `scripts/verify-code-only.mjs`(20축).
  🔴 셋이 각자 만들어 **셋 다 틀렸다**(B2 변이표 44줄 누수 · B2 zip 검사 거짓 빨강 · C zip 검사 주석 안 걷음).
- 🔴 **급한 정도는 «틀렸을 때 나는 소리»로 갈린다**: 헛 빨강(사람이 본다) < **조용한 초록**(아무도 안 본다).
  둘 다 «지금은 안 물린다»여도 **뒤쪽을 먼저** 고친다.
- ⚠️ **내가 보고에서도 그 병에 걸렸다**: 변이 결과를 `head -3` 으로 **잘라 보고** «셋»이라 적었는데 실제는 **넷**이었다.
  **수를 보고할 거면 자르지 마라.**

### 🔴 사장님 손 대기 — **넷**이다(2026-09-19 · 내가 직접 재서 고침)

| 무엇 | 누구 |
|---|---|
| **당근 탐침 1회** | 사장님 + B2 (체크리스트 §5.3) |
| **틱톡 키** | 사장님 |
| `video_90` **42코인** 임시값 결재 | 사장님 (B 가 올림) |
| 영상 레퍼런스 **월 한도 3/6/20/60** 결재 | 사장님 (B 가 올림) |

🔴 **러너 새 판은 끝났다**(v1.4.0 · 2026-09-19 02:24 KST).
🔴 **DDL `0081`·`0082` 는 이 목록에 없다 — 이미 걸려 있다.** 내가 `information_schema` 를 직접 읽었다:
`accounts.reader = character varying(120) · NULL 허용` · `channel_registry.daangn = 당근 · text · runner · planned · monetize=[]`.

⚠️ **내가 이틀 동안 «미적용»으로 들고 다니다 사장님 결재 목록에 실어 보냈다.**
출처는 **B 가 2026-09-17 에 메시지로 준 한 줄**이었고, **B 자기 상태 파일은 그 뒤 «닫혔다»로 고쳐져 있었다.**
🔴 **질의 한 줄이면 됐다.** «알고 싶은 것 = 라이브에 컬럼이 섰나» ↔ «센 것 = 내 문서에 미적용이라 적혀 있나» —
**문서가 라이브의 제일 손쉬운 대용물**이다(PITFALLS AC-109 ② 의 «소스 판 번호»와 같은 자리 · 덧3 ⓑ).
⇒ 🔴 **«사장님께 올릴 목록»은 올리기 전에 한 줄씩 재라.** 남이 준 목록을 그대로 나르지 마라.

### ⚠️ 🔴 `eof_action` 건의 **급한 정도** — 피해 입은 사람은 **0명**이었다(2026-09-19 · 내가 DB 로 셈)

내가 «**고객 영상에 제휴 고지가 안 실리고 있었다**»로 적어 사장님께 올렸는데, **라이브를 세어 보니**:

| | |
|---|---|
| 테넌트 | **109곳 전부 내부용** · 바깥 **0곳** |
| 발행된 글·영상 | 🔴 **라이브 전체에서 0건**(`published_at`·`external_url` 둘 다 없음) |
| 코인 주문 | **0건** |

⇒ 🔴 «고객이 겪고 있었다»가 아니라 «**고객이 생기기 전에 잡았다**»가 맞다.
잡은 것과 AC-33(심사가 계획서를 본다)은 **그대로 값지다** — 바뀌는 건 **급한 정도의 표현**뿐이다.
⚠️ **없는 피해를 있다고 하는 것도 «겁주기»다**(CLAUDE §3). 이 병은 **작게 세는 쪽으로만 틀리지 않는다.**
⇒ **«무엇이 걸렸나»와 «누가 겪었나»를 나란히 적어라.**

### 🔴 그리고 «내 것이 main 에 갔나»도 내가 세야 한다

내가 «제 브랜치가 main 에 없다 · 코드와 배포가 어긋나 있다»고 두 번 말했는데 **틀렸다.**
`git log main..HEAD` 를 **한 번** 재서 «7커밋»을 얻은 뒤 **다시 안 재고** 8·10 으로 더했고,
그 사이 메인이 머지해 **분모가 움직였다.** 실제로 남은 건 **문서 1커밋**이고 `main` 의 `runner/package.json` 은 **이미 `1.4.0`**(= 라이브와 같은 판).
🔴 **머지는 «내가 하는 것»이 아니라 «받는 쪽»이 한다** — 그래서 «내가 안 보냈으니 없다»가 틀린다.
🔴 **한 번 잰 수는 «그때의 수»다** — 움직이는 것을 세었으면 **쓸 때 다시 센다**(`git log main..<브랜치>` 한 줄).
