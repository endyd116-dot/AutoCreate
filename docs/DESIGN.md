...
|---|---|---|---|---|
| **유튜브 쇼츠** | YPP 쇼츠 광고수익(애드센스) · 유튜브 쇼핑 제휴 · 설명란 쿠팡 | **API**(Data API v3 `videos.insert`) | **앱 심사 전 업로드는 비공개 고정** · 일 쿼터 10,000u(업로드 1,600u ≈ **6건/일/프로젝트** = 이론 천장) · 🔴 **운영 기본값은 5건**(사장님 결정 2026-09-24) — 쿼터가 **채널이 아니라 구글 프로젝트** 단위라 고객 전부가 한 통을 나눠 쓰고, `videos.insert` 말고 조회·분석 호출도 같은 통을 쓴다. **여유 없이 6으로 두면 여섯 건째가 조용히 실패한다.** 정본은 `lib/publish/youtube.ts youtubeDailyCap()` 한 곳(env `YOUTUBE_DAILY_INSERT_CAP` 로 조절) · 화면은 그 값을 **받아서** 말한다(숫자를 베껴 적지 않는다 · AC-52). 늘리려면 **구글에 쿼터 증설 신청** | P1 |
...
| 링크프라이스 | 제휴 | API | 부분 |
| 텐핑 · 애드픽 · 네이버 쇼핑커넥트 | 제휴 | 🔴 **수동만**(2026-09-15 B2 확인) | 이름만 |

> 🔴 **2026-09-15 정정**: 텐핑·애드픽·쇼핑커넥트는 **공개 API 가 없고**(링크프라이스와 다르다) 관리자 화면도 본 적이 없어 **스크랩도 못 쓴다.**
> ⇒ 지금은 **수동 입력 소스**로 둔다. 다만 **이름을 갖게** 해서 «그 외»로 뭉개지던 것을 **소스별로 센다**(`manualSources`).
> 길이 생기면 `FRESHNESS_OF` 한 줄만 바꾸면 커넥터가 된다 — **«커넥터가 있다»고 적어 두지 않는다**(있는 척이 제일 나쁘다).
...
### 5B.11 🔴 잠깐 멈춤 (사장님 지시 2026-09-22 — «근본적인 잠깐 멈춤 기능»)

> 사장님: «고객 입장에서 잠시 멈추고 싶을 수 있는데 그 기능이 없어? 이건 추가해야 하지 않아?»
> 그리고 **«근데 크론 멈췄다는 걸 내가 까먹으면 어떡해?»** — 🔴 **그 물음이 이 설계의 핵심이다. 멈춤에는 «깨워 주기»가 같이 있어야 한다.**

**왜 없었나 — 지금 있는 멈춤은 전부 «나쁜 일이 생겨서» 멈추는 것이다**

| 지금 있는 것 | 누가 정하나 | 되돌리기 |
|---|---|---|
| `readonly`(체험 종료·탈퇴 신청) | 계약 | 결제해야 |
| `suspended` | **우리**(운영자) | 우리가 푼다 |
| 슬롯 `paused`(코인 부족 · §7.3) | **자동** | 충전하면 |
| 편성표 «이날은 쉬기» · `quietDays[]`(§5B.2) | 고객 | 하루/날짜 단위 |

🔴 **«손님이 스스로, 기간을 정해, 언제든 되돌릴 수 있게» 멈추는 자리가 하나도 없다.**
지금은 계정을 하나씩 **연결 해제**하거나 편성 규칙을 **0** 으로 만드는 수밖에 없고, **둘 다 되돌릴 때 원래대로 복구되지 않는다.**
⇒ 🔴 **«잠깐 멈추고 싶은데 방법이 없어서 해지»가 제일 나쁜 결말이다.**

#### (1) 무엇이 멈추고 무엇이 안 멈추나 — 🔴 «멈춤»이 «잠김»이 되면 안 된다

| 멈춘다 | 안 멈춘다(그대로 된다) |
|---|---|
| 자동 편성(`roll`·`assignTopics`) — **새 자리를 안 만든다** | 이미 만든 글 **보기·고치기** |
| 자동 제작(`produce`) — **코인이 안 나간다** | 🔴 **손으로 «지금 올리기»** — 고객이 누르면 나간다 |
| 자동 발행(`publisher`) — 예약된 것이 **안 나간다**(취소가 아니라 **대기**) | 수익 회수 · 정산 · 결제 · 알림 |
| | 러너 하트비트(기기는 계속 살아 있다) |

🔴 **예약된 글을 «버리지» 않는다.** 멈춤은 **보류**다.

#### (1-b) 🔴 깰 때 «밀린 글»을 어떻게 하나 — **우리가 정하지 않는다**(§9 · 2026-09-22 메인 결정)

> 이 자리는 처음 설계에 «지난 것은 흘려보낸다»라고만 적혀 있었다. **그 상태가 무엇인지 안 적었다.**
> B 가 `review-deadline` 을 멈출지 물으면서 그 구멍이 드러났다 — 🔴 **«설계에 없던 판단이라 적어 둔다»는 한 줄이 없었으면 영영 몰랐다.**

쉬는 동안 **발행 시각이 지나 버린 글**(예약돼 있었거나, 검수창이 닫혀 자동 승인된 것)은 아래 셋 중 **어느 것도 아니다**:

| 안 하는 것 | 왜 |
|---|---|
| ❌ **몰아서 올린다** | 2주 쉬고 켜면 **열 개가 한 번에** 나간다 — 계정이 다친다(§7.4 캐던스와 정면충돌) |
| ❌ **조용히 `skipped` 로 넘긴다** | 🔴 **손님이 만든 글이 말없이 사라진다.** 우리가 제일 오래 잡은 병이다 |
| ❌ **버린다(`rejected`)** | 손님이 버린 적이 없다. **우리가 대신 버리는 것**이다 |

✅ **«밀린 글»로 모아 두고 깰 때 고객이 고른다** — 새 상태(예: `pending_resume`). 🔴 `skipped`·`rejected` **재사용 금지**(뜻이 다르다).

```
다시 시작할 때
  «쉬는 동안 밀린 글이 N건 있어요»
  [ 지금부터 차례로 올릴게요 ]   [ 이건 그냥 둘게요 ]
```

- «차례로»는 **캐던스를 지켜서** 푼다(§7.4 `daily_cap`·`min_gap`) — 한 번에 쏟지 않는다.
- 🔴 **§9 그대로다: 우리가 판단하지 않는다. 말해 주고 고르는 것은 고객이다.**
- ⇒ 그래서 `review-deadline`(검수창 마감)은 **멈추지 않는다.** 멈추면 손님이 이미 만들어 둔 글이 **검수창에 갇힌 채 창이 닫힌다.** 자동 승인돼도 `publisher` 가 멈춰 안 나가고, 깰 때 **«밀린 글»로 손님 눈에 보인다**(B 판단 · 2026-09-22).
- 🔴 자: «쉬는 동안 자동 승인된 글이 **깰 때 손님 눈에 보이나**» — 안 보이면 그것이 «말없이 사라짐»이다.

#### (1-c) 응답 계약 (A 가 내고 B 가 받았다 · 2026-09-22)

```
GET /api/tenant-settings · GET /api/home-summary  →  pause: {
  paused, pausedAt, pauseUntil, reason,
  days,       // 🔴 «N일째» — 서버가 센다(KST · 첫날 = 1일째)
...
```
tenants.paused_at        timestamptz NULL   -- 언제부터 쉬나 (NULL = 안 쉰다)
tenants.pause_until      timestamptz NULL   -- 언제 저절로 깨나 (NULL = 내가 켤 때까지)
tenants.pause_reason     varchar(40) NULL   -- 고객이 고른 까닭(휴가·글 손보는 중·채널 제재·비용 아끼기·그 밖)
tenants.pause_notified_at timestamptz NULL  -- 🔴 마지막으로 «아직 쉬는 중» 을 알린 때
```
🔴 **`tenants.status` 를 건드리지 않는다.** `NON_WRITABLE`(`readonly`·`suspended`·`cancelled`)에 끼워 넣으면 **«쉼»이 «잠김»이 된다**(§0.1 `requireWritable` 는 한 곳이고 그 뜻은 «계약상 못 쓴다»다). 쉼은 **계약이 아니라 고객의 선택**이라 **별도 칸**이다.

#### (3) 🔴 깨워 주기 — 사장님 물음에 대한 답

| 언제 | 무엇 |
|---|---|
| **멈추는 순간** | «언제까지 쉴까요»를 **반드시 고르게 한다** — ①2주 뒤 자동 ②한 달 뒤 자동 ③**내가 켤 때까지** |
| ③을 고르면 | 🔴 **7일마다 «아직 쉬는 중이에요 · N일째»** 알림. `pause_notified_at` 로 멱등 |
| **홈 첫 화면** | 🔴 **«쉬는 중 · N일째»가 늘 보인다**(§13 첫 화면 숫자 1개 자리와 겹치면 그것이 이긴다). 까먹을 수가 없어야 한다 |
| **자동으로 깰 때** | 하루 전 «내일 다시 시작해요» · 깬 날 «다시 시작했어요» |
| **크론** | `pauseWatchStep`(hourly · `pause_until` 지난 집을 깨우고 ③의 7일 알림) |

#### (4) 화면 (§13.0b 패턴 = **설정 그룹 리스트 + 토글**)
- **끄기**: 설정 → «자동으로 만들고 올려요» 토글 → 바텀시트(한 화면 한 질문) «언제까지 쉴까요» + 까닭(선택) → «쉬기»
- **쉬는 중**: 그 자리에 «지금은 쉬고 있어요 · 9월 22일부터 · 12일째» + **[다시 시작]** 한 개
- 🔴 **겁주지 않는다(§3)**: «멈추면 수익이 줄어요» 금지. 사실 한 줄(«예약된 글은 그대로 두고 기다려요») + 되돌릴 길(«언제든 다시 시작할 수 있어요»)
- 🔴 **끄기 전에 무엇이 멈추는지 세 줄**로 말한다(§9-① 또렷하게) — 위 (1) 표의 왼쪽

#### (5) 우리(운영)에게도 같은 값
🔴 시험 테넌트를 멈추려고 **크론을 통째로 끄면**(`netlify.toml`) 결제 청구·수익 회수까지 같이 죽고 **켜는 것을 잊는다**(사장님 물음 그대로).
⇒ 이 기능이 있으면 **그 집만 «쉼»** 으로 두면 된다. 운영센터에도 같은 스위치(감사 `high` · 사유 필수).

#### (6) 자
- `verify-pause-scope` — 🔴 (1) 표가 **코드와 같은가**: 멈춘 집에서 «안 멈춘다» 칸이 실제로 되는가(**«멈춤»이 «잠김»이 되지 않았는가**)
- `verify-pause-wakes` — 🔴 ③을 고른 집에 **7일마다 알림이 가는가** · `pause_until` 이 지나면 **깨는가**(사장님 «까먹으면?» 의 자)
- 🔴 **`NON_WRITABLE` 에 `paused` 가 들어가면 운다** — 이 설계가 뒤집히는 자리를 자가 지킨다
...
🔴 **게이트 최소화(사장님 전역 지시 2026-09-15 · `CLAUDE.md §9`)** — **하드(막는다)는 «법·플랫폼 정책 위반»과 «되돌릴 수 없는 것»뿐**이다.
아래 축들은 **거의 전부 소프트**여야 한다(품질·모양·분량·반복). 지금 하드 = `disclosure` · `banned_words` · `ad_pointing` · `stock_safe`(법) + `affiliate_count` · `similarity`(**재검토 대상 — 법이 아니다**).
🔴 **`visualMin` 처럼 «몇 개 이상»을 요구하는 계약 값도 숨은 하드 게이트다** — 계약의 다른 부분(`tiers.optional`)과 싸우면 매번 재작성이 돌아 **돈이 두 배**로 든다.
...
> 정본 = `lib/disclosure.ts`(문구·위치 규칙) + `lib/ad-policy-gate.ts`(발행 직전 검사). 🔴 **위반해도 막지 않는다**(사장님 지시 2026-09-15 · `CLAUDE.md §9` — «말해 주기로 내려. 고객 계정이야»). 검사는 **계속 돌고 또렷하게 말한다** — 쿠팡은 위반 시 사후 통보로 수익 몰수, 애드센스는 계정 정지가 현실이니 **그 위험을 문장으로 준다.** 그리고 **고지 문장은 막는 게 아니라 넣어 준다**(`ensureDisclosureHtml` — 대가를 켜면 자동으로 첫머리에).
...
| **잠깐 멈춤**(§5B.11) | **그룹 리스트 + 토글** | 설정의 토글 하나 → «언제까지 쉴까요» 바텀시트(한 화면 한 질문) → 쉬는 동안은 그 자리가 «쉬는 중 · N일째 + [다시 시작]» 한 줄이 된다. 🔴 홈 첫 화면에도 같은 한 줄이 늘 보인다(까먹지 않게) |
...
🔴 **글자색 정정(2026-09-15 · A 가 WCAG AA 4.5:1 을 자로 재서 고침)** — «눈으로 보기에 옅다»가 아니라 **측정**이다. 수익 화면 **한 장에서만 21건 미달**이었고, 회색 글씨 전체가 **2.0~3.0:1** 이었다.

| 토큰 | 라이트 | 다크 | 무엇 |
|---|---|---|---|
| `--muted` | **#66707C** | (기존) | 보조 글자 — 흰 바탕 5.03 · 회색 바탕 4.56 |
| `--warn-ink` | **#965900** | (기존) | 경고 **글자** |
| `--money-ink` **신설** | **#127A57** | **#34C88F** | 수익 **글자** |
| `--danger-ink` **신설** | **#C32633** | **#FF6B75** | 위험 **글자** |

- 🔴 **채움색은 그대로다** — 막대·마크의 브랜드 초록 `#1FA97A`·빨강은 **면**이라 대비 기준이 다르다. **글자 전용 토큰을 따로 만든 것**이 이 수리의 핵심이다(같은 색을 글자와 면에 같이 쓰면 둘 중 하나가 반드시 진다).
- 🔴 **`--faint` 는 글자로 쓰지 않는다** — 다크에서 **2.71:1** 이라 글자로는 못 쓴다. 글자로 쓰던 자리는 전부 `--muted` 로 옮겼다.
- **검증**: shot 감사가 **전 화면(80여 장 × 폰·데스크톱)** 을 매번 다시 잰다. 눈으로 보지 않는다.
...
| 네이버 클립 PC 업로드 불가 | Android 에뮬(관리형 러너)로 대체 · 실패 시 «앱에서 올리기» 폴백 — 🔴 **딥링크는 아직 안 만든다**(2026-09-16 · B2 판단 · 메인 확정): **URL 스킴을 지어내지 않는다.** 네이버 클립 앱의 실제 스킴을 우리가 **한 번도 확인한 적이 없고**, 지어내면 «눌렀는데 아무 일도 안 남»이 된다. 지금은 «앱으로 열려요» 없이 **mp4 내려받기 + «올렸어요» 적기**(`lib/manual-upload.ts` · `appOpenVerified`)까지가 길이다. ⇒ **합동 세션에서 사장님이 클립 계정을 만들고 앱 화면을 한 번 열어 주시면** 그때 실제 스킴을 보고 만든다. 그때까지는 **«없는 길»**이다(CLAUDE §9). |
