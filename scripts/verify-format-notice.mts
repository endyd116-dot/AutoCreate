...
console.log("\n[②b 🔴 되돌릴 길 — 채널 세 갈래]");
{
  ...
  /* ═══ 🔴 N-16 — **«표에 없는 채널»**(C 발견 2026-09-16 · B 수리) ═══
   *   내 첫판은 «채널을 **안 준** 경우»만 표본에 넣었다. 그런데 코드에는 가지가 **둘**이다:
   *     ① 채널을 안 줌   ② 채널을 줬는데 **레지스트리 표에 없음**
   *   ②가 `canRetract` 의 2값(«표에 없음»과 «표에 있고 null»이 같은 답)에 걸려
   *   **«채널에서 직접 바꾸세요»** 로 떨어지고 있었다 — 「모른다」를 「못 한다」로 읽은 것이다(AC-92).
   *   🔴 **한쪽만 표본에 넣어 무력화가 안 보인 자리**다(AC-99 ⑨). C 가 표본을 넓게 잡아서 보였다.
   *   ⚠️ 언제 나나: **새 채널을 `pieces` 에 먼저 넣고 레지스트리 행을 나중에 넣을 때** — 채널을 늘릴 때마다 생기는 틈이다. */
  const notInTable = say("zzz_unknown");
  ok(!/바꾸실 수 있어요|다시 올릴 수 있어요|내렸다가/.test(notInTable),
    "N-16 🔴 **표에 없는 채널**도 되돌릴 길을 지어내지 않는다(«모른다» ≠ «못 내린다» · AC-92)", notInTable);
  ok(notInTable === unknown,
    "N-16b 🔴 «표에 없는 채널»과 «채널 안 줌»이 **같은 답**이다(둘 다 «모른다»니까)", `표밖=«${notInTable}» · 안줌=«${unknown}»`);
  /* 🔴 대조군 짝 — **표에 있고 진짜 못 내리는 채널**은 여전히 «채널에서 직접»이라고 말해야 한다.
     안 그러면 «모름»을 고치다 «아는 것»까지 입을 닫게 된다(고치면서 반대쪽을 깨뜨리지 않았다는 증거 · AC-68). */
  ok(/채널에서 직접/.test(say("naver_clip_post")),
    "N-16c 대조군 짝 — **표에 있고 `retractVia: null`** 인 채널은 여전히 «채널에서 직접»이라고 말한다(«모름»을 고치다 «아는 것»을 잃지 않았다)", say("naver_clip_post"));
}
