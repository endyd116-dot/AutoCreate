...
const THREE_REST = [
  ...
  /* 🔴 [2026-09-16 · C 자체검사] 아래 다섯 칸은 한국어 낱말(«적합도»·«신조어»·«장소 카드»·«목표 매체»·«동기화 PR»)로 재고 있었다.
     지금은 매치가 0 이라 정직하지만, «신조어 화이트리스트는 아직 없다» 같은 **주석 한 줄**이면 내일 닫힘으로 뒤집힌다.
     B2 가 B5(쓰레드 연결글)에서 찾은 병이 정확히 그것이고, 같은 훑기에서 B6·E2 둘이 실제로 그랬다.
     ⇒ **식별자 모양**(코드)만 센다 — 사람말은 주석에 나오고 식별자는 안 나온다. 만들면 이름이 생기니 그때 닫힌다. */
  ["B2 페르소나 적합도 LLM", () => [yes(anyFile(["lib/ai-tell-gate.ts", "lib/content-approve.ts"], /personaFit\s*[(=:]/).length), "0건(주석 말고 **코드**로 센다)"]],
  ["B3 신조어 화이트리스트", () => [yes(anyFile(["lib/ai-tell-gate.ts", "lib/banned-words.ts"], /slangWhitelist\s*[(=:]/).length), "0건(주석 말고 **코드**로 센다)"]],
  ["B4 장소 카드", () => [yes(anyFile(["lib/blocks.ts", "lib/writing-contracts.ts"], /placeCard\s*[(=:?]|"placeCard"/).length), "0건(주석 말고 **코드**로 센다)"]],
  ...
  ["B8 목표 매체 채널 선택", () => [yes(inFile("lib/director.ts", /targetChannel\s*[(=:?]/)), "0건(주석 말고 **코드**로 센다)"]],
