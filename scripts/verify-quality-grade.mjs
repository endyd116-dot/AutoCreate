 ...
 *
 *   ══ 🔴 **이 등급이 얼마나 참인가 — 반드시 같이 읽어라**(2026-09-23 · AC-234·235) ══
 *     ① **「100점」은 «다 좋다»가 아니라 «이 축들은 좋다»다.** 사장님이 **100점짜리 화면에서 눈으로 둘을 찾으셨다**
 *        (`.fr` 왼쪽 어긋남 · 계정 화면 순서). 자가 틀린 게 아니라 **모수가 좁았다** — 그래서 A 의 축 둘을 넣었다.
 *        🔴 **점수가 떨어질까 봐 축을 안 넣는 것이 이 자를 죽이는 길이다.** 새 축은 넣고, 떨어지면 떨어뜨린다.
 *     ② 🔴 **넘기는 축 7개 중 다섯이 «모의»(`public/js/mock.js`)를 본다** — `people-words`·`hand-on-button`·
 *        `r8-deadends`·`left-edge`·`first-screen`. ⇒ **이 등급의 절반은 «모의만큼만» 참이다.**
 *        **쓰는 곳 수**: `people-words` 14 · `hand-on-button` 17 · `r8-deadends` 4 · `left-edge` 1 · `first-screen` 1.
 *        🔴 **A 가 재서 밟았다**(2026-09-23 · 내가 재진 것이 아니다): 모의는 채널 **11칸**이라 4줄이 첫 화면에 들어왔는데 라이브 **17칸**에서만 안 들어왔다
 *        — **옛 순서로 되돌려도 변이가 안 울었다.** 🔴 **모의로 못 재는 자는 자가 아니다.**
 *        ⚠️ 라이브 대비 모의가 얼마나 작은지는 **내가 못 쟀다**(정적 뽑기가 두 번 어긋났다) — A 의 실측을 쓴다(AC-9).
 *     ③ **표본이 작은 자는 «초록»보다 «표본»을 먼저 봐라** — `verify-left-edge` 는 기본 표본이 **3화면**이다
 *        (`PAGES=` 로 넓힌다 · 내가 15화면까지 넓혀 봤고 어긋남 0 이었다).
 *
 *   ══ 등급 ══
...
/* ── 넘기는 축 ────────────────────────────────────────────────────────── */
const HANDOFF = [
  /* 🔴 [2026-09-23 · AC-234] **A 가 낸 축 둘을 등급에 넣는다.**
     사장님이 «품질 100점»인 화면에서 **눈으로 둘을 찾으셨다**(`.fr` 왼쪽 어긋남 · 계정 화면 순서).
     🔴 **내 자가 틀린 게 아니라 모수가 좁았다** — 「100점」은 «다 좋다»가 아니라 **«그 17가지는 좋다»**였다.
        오늘 종일 잡은 그 병(모수)이 **내 등급표에** 있었던 것이다.
     🔴 **넣으면 점수가 떨어질 수 있고, 떨어지면 떨어뜨린다** — 100점을 지키려고 축을 안 넣는 것이
        우리가 오늘 막은 바로 그 짓이다. (재 보니 A 가 이미 고쳐 둘 다 초록이라 점수는 그대로였다.) */
  ["`.group` 안 왼쪽 시작점이 같나", "scripts/verify-left-edge.mjs"],
  ["첫 화면에 그 기능 입구가 있나", "scripts/verify-first-screen.mjs"],
...
if (JSONOUT) { console.log(JSON.stringify({ rows, grid: gridNote }, null, 2)); }
  ...
  /* 🔴 **점수 바로 다음에 눈으로** 붙인다 — 메인 지적(2026-09-23): 이 단서를 머리말 주석에만 적었더니
     **100점을 읽는 사람은 못 봤다.** 주석은 «지나가는 회색 글씨»보다도 안 보인다(CLAUDE §9 ① «또렷하게 말한다»).
     🔴 그리고 **출처를 같이 찍는다** — 숫자만 있고 «누가 재는가»가 없으면 다음 사람이 그걸 제가 재서 안 것처럼 쓴다(AC-217·AC-238). */
  const mockAxes = [["people-words", 14], ["hand-on-button", 17], ["r8-deadends", 4], ["left-edge", 1], ["first-screen", 1]];
  console.log(`   ⚠️ 🔴 **이 점수의 절반은 «모의만큼만» 참이다** — 넘기는 축 ${HANDOFF.length} 중 **${mockAxes.length}개**가 모의를 본다`);
  console.log(`      (${mockAxes.map(([a, c]) => `${a} ${c}`).join(" · ")} — 괄호 안은 그 자가 모의를 언급한 곳 수)`);
  console.log(`      라이브 채널은 **17**, 모의는 **11** — 🔴 **A 실측이다(내가 재진 것이 아니다).**`);
  console.log(`      ⇒ 모의가 라이브보다 작으면 **이 등급도 그만큼만 참이다**(AC-238).`);
