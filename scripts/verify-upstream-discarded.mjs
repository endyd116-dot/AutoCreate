 ...
 *     · 🔴 **`pieces.meta` **하나만** 본다** — C 실측 2026-09-21: `db/schema.ts` 에 jsonb 칸이 **43개**인데
 *       **나머지 42개의 «안»은 이 자도, 짝(`verify-write-path-missing`)도 안 본다.** 그 안에 이름 붙은 계약이 있다 —
 *       `ops_settings.features`(영상 킬 스위치와 같은 갈래인데 jsonb 키) · `posts.stats` · `tenants.settings` · `personas.profile`.
 *       (`runner_jobs.payload`·`result` 처럼 **불투명한 짐**은 «키 계약»이 아니라 대상이 아니다.)
 *       ⇒ 🔴 **«이 쌍이 전부를 덮는다»고 읽지 마라.** 아직 안 만든 이유는 짝 자의 머리말에 적어 뒀다.
 ...
 *   ⚠️ 글자 훑기다(파서 아님): 따옴표·템플릿 한 겹은 건너뛰지만 **템플릿 안의 템플릿**·**정규식 리터럴 안 괄호**는 모른다(`code-only.mjs` 한계와 같다).
 *      삼항 `a ? b : c` 의 b 를 키로 세는 것도 종전 그대로다(오탐 쪽 · 지금 meta 쓰기엔 0곳).
 */
function keysSource(s, m) {
  ...
  for (let i = open; i < s.length; i++) {
    ...
    if (c === "{" || c === "(" || c === "[") { depth++; if (depth > 1) out += c; continue; }
    if (c === "}" || c === ")" || c === "]") { depth--; if (depth === 0) break; out += c; continue; }
    out += (c === ":" && depth > 1) ? " " : c;
  }
  return out;
}
