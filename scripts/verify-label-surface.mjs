   ...
   🔴 B 가 머지되기 전엔 서버 표가 없다 — 그때는 △(경고)로 두고, 표가 생기면 **글자·숫자까지** 대조한다. */
{
  ...
  /* [A · 2026-09-16] 정본 둘: lib/blocks.ts `export const MARK_LABEL`(마크 여섯) + lib/format-marks.ts `export const FIELD_LABEL`(블록 요소 · MARK_LABEL 을 펼쳐 넣는다).
     🔴 이름만 찾으면 주석의 «MARK_LABEL·FIELD_LABEL» 에 먼저 설 수 있다(⑧-c 의 «0등급»과 같은 병) — `export const` 에 닻을 내린다.
     모의 FMT_UNUSED 의 label 은 서버 글자여야 하고, why 는 서버가 사람말 문장(WHY_SAY)으로 실어 주므로 모의 문장도 그 표의 값이어야 한다(시연이 곧 고객 문장). */
  ...
  if (!markSrv.size) rec("⑧-d 서식 이름표 — 서버 MARK_LABEL·FIELD_LABEL 을 못 읽었다", "WARN", `화면 사본 ${markUi.size}개 · 모의 ${fmtMock.length}칸`);
  else {
