 ...
 *   칸마다 «그 축이 낼 말»을 적어 두고 **그 말이 나왔을 때만** 제대로 운 것으로 센다.
 *
...
const cases = [];
/* 🔴 **AC-121** — «울었다»만 보면 «공짜로 통과하는 변이»가 생긴다.
   변이 ⑦ 을 넣었는데 엉뚱하게 축 ② 가 울어도 종료코드는 똑같이 1 이다 ⇒ **⑦ 축은 눈이 먼 채로 초록**이 된다.
...
}, "정적 `#cta` 에 단추가 미리 박혀 있다");

/* ② 그리는 자리에서 `bindCta()` 호출만 뗀다 — «그려는 놓고 손은 안 붙인다» */
add("② `bindCta()` 호출을 떼면 운다", (b) => inSeg(b, (s) => s.replace(/;\s*bindCta\(\);/, ";")), "`bindCta` 호출 없음");

/* ③ `bindNow()` — 🔴 도우미(`nowBtn()`) 안 단추라 **펼쳐서 세지 않으면 안 운다**(A 가 한 번 속은 자리) */
add("③ `bindNow()` 호출을 떼면 운다 — 도우미 안 단추를 펼쳐서 세나", (b) => inSeg(b, (s) => s.replace(/;\s*bindNow\(\);/, ";")), "`bindNow` 호출 없음");
...
add("⑦ `EDITABLE_ST` 에서 `edited` 를 빼면 운다(서버는 승인해 주는데 화면이 안 보여 준다)", (b) =>
  inSeg(b, (s) => s.replace(/(EDITABLE_ST\s*=\s*\[)([^\]]*)\]/, (m, a, list) => a + list.replace(/"edited"\s*,?\s*/, "") + "]")), "서버는 [edited] 도 승인해 주는데");
...
for (const c of cases) {
  ...
  const ok = planted && cried && rightAxis;
  if (!ok) bad++;
  const why = !planted ? "🔴 **변이를 못 넣었다**(바뀐 줄 0) — 자를 잰 게 아니다"
    : !cried ? `바뀐 줄 ${r.changed} · 🔴 **안 울었다** — 이 자리는 초록으로 지나간다`
      : !rightAxis ? `바뀐 줄 ${r.changed} · 🔴 **울긴 울었는데 딴 축이 울었다** — «${c.expect}» 가 안 나왔다(AC-121)`
        : `바뀐 줄 ${r.changed} · 울었다`;
